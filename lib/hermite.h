...
float dx(int i, float *t, float Tens, float Bias, float Cont, Shape *Fig) {
    ...
    if (i % 2 == 0)
        return 0.5 * (1 - Tens) * (1 + Bias) * (1 + Cont) * (Fig->CP.at(i).x - Fig->CP.at(i - 1).x) /
               (t[i] - t[i - 1]) +
...
float dy(int i, float *t, float Tens, float Bias, float Cont, Shape *Fig) {
    if (i == 0)
        return 0.5 * (1.0 - Tens) * (1.0 - Bias) * (1 - Cont) * (Fig->CP.at(i + 1).y - Fig->CP.at(i).y) /
               (t[i + 1] - t[i]);
    if (i == Fig->CP.size() - 1)
        return 0.5 * (1 - Tens) * (1 - Bias) * (1 - Cont) * (Fig->CP.at(i).y - Fig->CP.at(i - 1).y) / (t[i] - t[i - 1]);

    if (i % 2 == 0)
        return 0.5 * (1 - Tens) * (1 + Bias) * (1 + Cont) * (Fig->CP.at(i).y - Fig->CP.at(i - 1).y) /
               (t[i] - t[i - 1]) +
...
float DX(int i, float *t, Shape *Poligonale, Shape *Derivata) {
...
float DY(int i, float *t, Shape *Poligonale, Shape *Derivata) {
...
void InterpolazioneHermite(float *t, Shape *Curva, Shape *Poligonale, Shape *Derivata, glm::vec4 color_top,
                           glm::vec4 color_bot) {
    ...
    for (tg = 0; tg <= 1; tg += passotg) {
        ...
        x = Curva->CP[is].x * PHI0(tgmapp) + DX(is, t, Poligonale, Derivata) * PHI1(tgmapp) * ampiezza +
            Curva->CP[is + 1].x * PSI0(tgmapp) + DX(is + 1, t, Poligonale, Derivata) * PSI1(tgmapp) * ampiezza;
        y = Curva->CP[is].y * PHI0(tgmapp) + DY(is, t, Poligonale, Derivata) * PHI1(tgmapp) * ampiezza +
            Curva->CP[is + 1].y * PSI0(tgmapp) + DY(is + 1, t, Poligonale, Derivata) * PSI1(tgmapp) * ampiezza;
        Curva->vertices.push_back(glm::vec3(x, y, 0.0));
